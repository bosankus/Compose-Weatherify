...
fun ForecastDto.toForecastTestResponse(): ForecastTestResponse {
    return ForecastTestResponse(
        cod = cod, cnt = cnt, windSpeed = list?.get(0)?.wind?.speed, cityName = city?.name
    )
}

data class ForecastTestResponse(
    val cod: String? = "",
    val cnt: Int? = 0,
