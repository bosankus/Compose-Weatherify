...
internal fun Long.dayName(): String {
    ...
    return localDateTime.dayOfWeek.name.lowercase().replaceFirstChar { it.uppercaseChar() }
}
...
internal fun getFormattedDateTimeFromEpoch(epoch: Long?): String {
    ...
    val dayAbbrev = localDateTime.dayOfWeek.name.take(3).lowercase().replaceFirstChar { it.uppercaseChar() }
    val monthAbbrev = localDateTime.month.name.take(3).lowercase().replaceFirstChar { it.uppercaseChar() }
