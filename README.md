...
```text
app/
├── androidApp/                             — plain Android application module (Hilt + Koin)
│   ├── build.gradle.kts
│   ├── google-services.json                — Firebase config (Analytics/FCM), not committed
│   ├── proguard-rules.pro
│   └── src/main/
│       ├── AndroidManifest.xml
│       ├── java/bose/ankush/weatherify/
│       │   ├── WeatherifyApplication.kt    — initKoin() + Hilt @HiltAndroidApp
│       │   ├── base/
│       │   │   ├── common/InAppUpdateManager.kt
│       │   │   └── notification/           — NotificationHelper, WeatherifyMessagingService (FCM)
│       │   ├── di/                         — Hilt/Koin bridge modules
│       │   │   ├── AppModule.kt
│       │   │   ├── NetworkModule.kt        — bridges Koin-registered network deps into Hilt
│       │   │   ├── StorageModule.kt        — bridges Koin-registered storage deps into Hilt
│       │   │   └── PaymentKoinModule.kt    — Koin module providing PaymentConfig (name is legacy)
│       │   └── presentation/
│       │       └── MainActivity.kt         — hosts AppNavigation(...) via setContent
│       └── res/                            — launcher icons, per-locale strings, splash/theme XML
│
└── iosApp/                                 — KMP module (iosArm64 + iosSimulatorArm64, Koin only)
    ├── build.gradle.kts
    ├── iosApp.podspec
    ├── Podfile / Podfile.lock              — CocoaPods deps (Firebase, Razorpay)
    ├── project.yml                         — XcodeGen source of truth for the Xcode project
    ├── iosApp.xcodeproj/ , iosApp.xcworkspace/   — generated by `xcodegen generate` (gitignored)
    ├── src/iosMain/kotlin/bose/ankush/iosapp/
    │   ├── Koin.kt                         — startWeatherifyKoin(), exported to Swift as KoinKt.startWeatherifyKoin()
    │   ├── MainViewController.kt           — ComposeUIViewController { ... } entry point
    │   └── payment/RazorpayCheckoutBridge.kt
    └── iosApp/                             — native Swift package (Xcode target root)
        ├── iOSApp.swift                    — @main App, calls FirebaseApp.configure()
        ├── ContentView.swift               — hosts MainViewController via UIViewControllerRepresentable
